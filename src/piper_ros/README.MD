...
节点名`piper_master_slave_bridge_node.py`

注意事项

- 主臂/从臂角色需要提前按`piper_sdk`官方方式配置完成，本节点不会自动调用主从模式切换指令
- 默认使用两个独立的 CAN 口，例如`can0`连接主臂，`can1`连接从臂
- 如果主臂控制帧超时，节点会向从臂发送一次停止指令并锁存故障，需重启节点后才能恢复

param

```shell
master_can_port: 主臂连接的 CAN 路由名，默认 can0
slave_can_port: 从臂连接的 CAN 路由名，默认 can1
bridge_rate_hz: 桥接轮询频率，默认 200
command_timeout_sec: 主臂控制帧超时阈值，默认 0.2
auto_enable_slave: 检测到首个有效主臂控制帧后是否自动使能从臂，默认 true
follow_gripper: 是否同步转发主臂夹爪控制，默认 true
```

`start_master_slave_bridge.launch`如下:
...
<launch>
  ...
  <arg name="command_timeout_sec" default="0.2" />
  <arg name="auto_enable_slave" default="true" />
  <arg name="follow_gripper" default="true" />

  <node name="piper_master_slave_bridge_node" pkg="piper" type="piper_master_slave_bridge_node.py" output="screen">
...
```shell
roslaunch piper start_master_slave_bridge.launch master_can_port:=can0 slave_can_port:=can1
```

若需要反向桥接，请先重新配置主臂/从臂角色，再交换`master_can_port`和`slave_can_port`参数后重启节点。
