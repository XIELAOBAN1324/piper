...
class C_PiperInterface_V2():
    ...
    class ArmCtrlCode_150():
        '''
        机械臂发送控制指令0x150的消息接收,由主臂发送
        '''
        '''
        The control command message 0x150 is sent by the main arm for reception
        '''
        def __init__(self):
            self.time_stamp: float=0
            self.Hz: float = 0
            self.ctrl_150=ArmMsgMotionCtrl_1()
        def __str__(self):
            return (f"time stamp:{self.time_stamp}\n"
                    f"Hz:{self.Hz}\n"
                    f"{self.ctrl_150}\n")
    ...
    def __init__(self,
        ...
        self.__fps_counter.add_variable("ArmCtrlCode_150")
        ...
        self.__arm_ctrl_code_150_mtx = threading.Lock()
        self.__arm_ctrl_code_150 = self.ArmCtrlCode_150()
    ...
    def ParseCANFrame(self, rx_message: Optional[can.Message]):
        ...
        if(receive_flag):
            ...
            self.__UpdateArmCtrlCode150(msg)
    ...
    def GetArmCtrlCode150(self):
        '''
        Retrieves the 0x150 control command, which carries emergency-stop, trajectory,
        and drag-teach control commands from the master arm.
        ...
        Hz : float
            msg fps
        ctrl_150 : ArmMsgMotionCtrl_1

            - emergency_stop (int): Emergency-stop command.
            - track_ctrl (int): Trajectory control command.
            - grag_teach_ctrl (int): Drag-teach control command.
        '''
        with self.__arm_ctrl_code_150_mtx:
            self.__arm_ctrl_code_150.Hz = self.__fps_counter.get_fps("ArmCtrlCode_150")
            return self.__arm_ctrl_code_150

    def GetArmMotionCtrl1(self):
        '''
        Alias for GetArmCtrlCode150().
        '''
        return self.GetArmCtrlCode150()
    ...
    def __UpdateArmCtrlCode150(self, msg:PiperMessage):
        '''
        更新主臂发送的150控制指令

        0x150
        '''
        '''Updates the control command 0x150 sent by the main arm.

        Args:
            msg (PiperMessage): The input containing the summary of robotic arm messages.
        '''
        with self.__arm_ctrl_code_150_mtx:
            if(msg.type_ == ArmMsgType.PiperMsgMotionCtrl_1):
                self.__fps_counter.increment("ArmCtrlCode_150")
                self.__arm_ctrl_code_150.time_stamp = msg.time_stamp
                self.__arm_ctrl_code_150.ctrl_150.emergency_stop = \
                    msg.arm_motion_ctrl_1.emergency_stop
                self.__arm_ctrl_code_150.ctrl_150.track_ctrl = \
                    msg.arm_motion_ctrl_1.track_ctrl
                self.__arm_ctrl_code_150.ctrl_150.grag_teach_ctrl = \
                    msg.arm_motion_ctrl_1.grag_teach_ctrl
            return self.__arm_ctrl_code_150
    ...
    def __UpdateArmCtrlCode151(self, msg:PiperMessage):
        ...
        with self.__arm_ctrl_code_151_mtx:
            if(msg.type_ == ArmMsgType.PiperMsgMotionCtrl_2):
                ...
                self.__arm_ctrl_code_151.ctrl_151.installation_pos = \
                    msg.arm_motion_ctrl_2.installation_pos
            return self.__arm_ctrl_code_151
        ...
        with self.__arm_mode_ctrl_mtx:
            if(msg.type_ == ArmMsgType.PiperMsgMotionCtrl_2):
                ...
                self.__arm_mode_ctrl.mode_ctrl.residence_time = \
                    msg.arm_motion_ctrl_2.residence_time
                self.__arm_mode_ctrl.mode_ctrl.installation_pos = \
                    msg.arm_motion_ctrl_2.installation_pos
            return self.__arm_mode_ctrl
